...
WHERE p.country_code = 'ua'
  AND p.group_name = '{PARTNER_NAME}'
  AND f.metric_timestamp_local >= '{STORE_WEEKS_START}'
  AND f.metric_timestamp_local <= '{WEEKLY_END}'
GROUP BY 1, 2
ORDER BY 1, 2
"""
...
def main():
    ...
    print("Fetching store-level ratings...")
    store_ratings = to_serializable(run_query(cursor, STORE_RATINGS_WEEKLY))

    # Merge ratings into store_weekly
    rating_map = {}
    for r in store_ratings:
        key = (r["period"], r["provider_name"])
        rating_map[key] = r.get("avg_rating")
